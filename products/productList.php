            ...
            if (strip_tags(trim($_POST["email"])) == $_SESSION["email"]) {
                if (isset($_SESSION["cart"])) {
                    ...
                    foreach ($cart as $key => $value) {
                        $productHistoryQuery = 'insert into productHistory(userId, productId) values(:userId, :productId)';
    ...
    if ($product && $sentencia->rowCount() > 0) {
        foreach ($product as $row) {
            ...
            <div class="card" style="width: 18rem; margin:10px;">
                <img style="width:100%;height:262px;object-fit:cover" class="card-img-top" src='<?php echo $row["picture"]; ?>'>
                <div class="card-body">
                    <h5 class="card-title">
                        <?php
